...
├── routes/                     # Route modules: commands, dashboard, directories, files,
...
| **Shaper** (`"shaper"`) | auto-compact, auto-inject-reminder, todo-continuation, transcript-save | Project Todo discussion agent. Investigates with read-only source tools and guarded Bash, updates only its bound Todo through `project_todo_update`, reads/writes Memory manually, and delegates research only to Explore/Librarian. No automatic memory extraction or execution-resource creation. |
