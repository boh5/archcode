// Shared product identity and public contract names.
// Keep these values branded: they are user-facing or persisted contracts, not generic placeholders.

export const PRODUCT_DISPLAY_NAME = "ArchCode";
export const PRODUCT_SLUG = "archcode";

export const CLI_BINARY_NAME = "archcode";
export const PROJECT_STATE_DIR_NAME = ".archcode";
export const PROJECT_RUNTIME_DIR_NAME = "runtime";
export const USER_DATA_DIR_NAME = ".archcode";
export const CACHE_NAMESPACE = "archcode";
export const MCP_CLIENT_NAME = PRODUCT_SLUG;

export const ENV_PREFIX = "ARCHCODE";
export const ENV_PORT = "ARCHCODE_PORT";
export const ENV_SERVER_PASSWORD = "ARCHCODE_SERVER_PASSWORD";
export const ENV_LOG_LEVEL = "ARCHCODE_LOG_LEVEL";
export const ENV_ACCESS_LOG = "ARCHCODE_ACCESS_LOG";
export const ENV_OPEN_BROWSER = "ARCHCODE_OPEN_BROWSER";
export const ENV_CLI = "ARCHCODE_CLI";

export const COOKIE_SERVER_PASSWORD = ENV_SERVER_PASSWORD;
export const HTTP_USER_AGENT = PRODUCT_DISPLAY_NAME;
