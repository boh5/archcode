...
describe("dynamic range compression", () => {
  ...
  test("protects running tools, unknown results, active questions, protect tags, child links, todos, and reminders", () => {
    const state = baseState([
      message("msg-1", "user", [text("t1", "<protect>keep this</protect>")]),
      message("msg-2", "assistant", [{ type: "tool", id: "tool-1", state: "running", toolCallId: "call-1", toolName: "bash", input: {}, createdAt: 1, startedAt: 2 }]),
      message("msg-3", "assistant", [{ type: "tool", id: "tool-2", state: "error", toolCallId: "call-2", toolName: "file_write", input: {}, errorMessage: "unknown", createdAt: 1, startedAt: 2, endedAt: 3, meta: { unknownResult: true } }]),
    ]);
    ...
    const result = prepareDynamicRangeCompression(state, { startId: "m0001", endId: "m0003", summary: summary() }, 1000);
