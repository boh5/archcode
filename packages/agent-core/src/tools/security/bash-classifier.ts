...
const RAW_DENY_RULES: RawDenyRule[] = [
  ...
  { ruleId: "deny-protected-permissions-file", reason: "Protected permission file access is blocked", pattern: /(?:\.\/)?\.specra\/permissions\.json/ },
  { ruleId: "deny-direct-specra-mutation", reason: "Direct mutation of .specra is blocked", pattern: /(^|[^\w.-])(rm|mv|cp|tee|mkdir|touch|chmod|chown)\b[^\n;&|]*\.specra(\/|\s|$)|(^|[^\w.-])git\s+clean\b[^\n;&|]*\.specra(\/|\s|$)|(^|\s)(>|>>|2>|2>>|&>|&>>)\s*\.specra\// },
];
