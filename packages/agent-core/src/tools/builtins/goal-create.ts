...
import { TOOL_GOAL_CREATE, type GoalState } from "@archcode/protocol";
...
export const GoalCreateInputSchema = z.strictObject({
  objective: GoalTextSchema.max(8_000).describe("Natural-language objective for the draft Goal."),
...
export const goalCreateTool: AnyToolDescriptor = defineTool({
  ...
  description: "Create a draft Goal from an ordinary Engineer session. Starting the Goal remains an explicit UI or server action.",
  ...
  execute: async (input: GoalCreateInput, ctx: ToolExecutionContext): Promise<string | ToolExecutionResult> => {
    try {
      assertGoalCreateAuthorized(ctx);
      const manager = ctx.projectContext.goalState as unknown as GoalDraftCreator;
      ...
      ctx.projectContext.onGoalCreated?.(goal);
