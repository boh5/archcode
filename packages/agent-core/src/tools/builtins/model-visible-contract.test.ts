...
const CONTRACTS: readonly ModelVisibleContract[] = [
  ...
  },
  {
    ...
    descriptionPatterns: [/status/, /terminal reminder/, /background_output\(/, /block=false/, /status is still running.*not a final deliverable/i, /full_session=true/, /since_message_id.*exclusive forward cursor/i],
    ...
    schema: [
      ...
      { path: ["properties", "full_session"], descriptionPatterns: [/filtered stored messages/, /final answer.*lacks needed evidence/i, /Default false/] },
      { path: ["properties", "message_limit"], descriptionPatterns: [/Maximum messages/, /Default 20/, /max 100/] },
      { path: ["properties", "since_message_id"], descriptionPatterns: [/Exclusive cursor/, /after this id/] },
      { path: ["properties", "include_tool_results"], descriptionPatterns: [/tool call results/, /default/i] },
