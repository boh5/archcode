...
const fixturePath = join(import.meta.dir, "../../package.json");
...
const tempRoot = join(import.meta.dir, "__test_tmp__", "builtin-standalone", crypto.randomUUID());
...
test("standalone binary preserves real builtin resources and arbitrary embedded bytes", async () => {
  ...
  const sourceSeedBytes = await Bun.file(fixturePath).bytes();
  const sourceFixtureBytes = appendInvalidUtf8Byte(sourceSeedBytes);
...
function standaloneSource(paths: {
  ...
  return [
    ...
    "",
    "const fixtureSeedBytes = await Bun.file(embeddedFixturePath).bytes();",
    "const fixtureBytes = new Uint8Array(fixtureSeedBytes.byteLength + 1);",
    "fixtureBytes.set(fixtureSeedBytes);",
    "fixtureBytes[fixtureSeedBytes.byteLength] = 0xff;",
    ...
    "    ].join(\"\\n\"),",
    "    resources: { \"assets/fixture.bin\": fixtureBytes },",
    "  },",
    "};",
...
function appendInvalidUtf8Byte(bytes: Uint8Array): Uint8Array {
  const result = new Uint8Array(bytes.byteLength + 1);
  result.set(bytes);
  result[bytes.byteLength] = 0xff;
  return result;
}
