...
3. Read the PRD artifact, record completion of product_drafting with workflow_record_completion, move to critic_prd_review with workflow_update_stage, then delegate Critic to review the PRD.
4. When Critic returns, use workflow_update_stage with the criticDecision parameter: "approved", "changes_requested", or "rejected". The stage field is still required but will be ignored when criticDecision is provided — pass the current stage as a placeholder.
...
6. Read SPEC/TASKS, record completion of spec_drafting with workflow_record_completion, move to critic_spec_review with workflow_update_stage, then delegate Critic to review SPEC/TASKS.
7. When Critic returns, use workflow_update_stage with the criticDecision parameter (same pattern as step 4).
...
10. After Foreman completes, read artifacts/reports, record completion of foreman_executing with workflow_record_completion, move to final_review, perform final verification/reporting, write the final report, record completion of final_review, then mark workflow status completed.
