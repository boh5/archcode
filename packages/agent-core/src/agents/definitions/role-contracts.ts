...
export const engineerRoleContract = {
  ...
  requiredBehaviors: [
    ...
    "Verify implementation and child reports before reporting completion.",
    "For an active Goal, delegate an independent Reviewer after the work is ready, read its final report, address requested changes, and complete only with the approved Reviewer Session ID.",
  ],
  forbiddenBehaviors: [
  ...
  requiredCapabilities: ["file_read", "delegate"], forbiddenCapabilities: ["project_todo_update"], allowedTransitions: [],
...
export const planRoleContract = {
  ...
  inputs: ["delegation objective", "owned scope", "repository evidence"],
  ...
  requiredCapabilities: ["file_read", "delegate"], forbiddenCapabilities: ["file_write", "file_edit"], allowedTransitions: [],
...
export const buildRoleContract = {
  ...
  inputs: ["delegation request", "owned scope", "repository evidence"],
  ...
  requiredCapabilities: ["file_read", "file_edit", "delegate"], forbiddenCapabilities: ["project_todo_update"], allowedTransitions: [],
...
export const reviewerRoleContract = {
  ...
  mission: "Independently verify one delegated body of work and return a decisive normal final report.",
  inputs: ["delegation objective", "attributable diff", "tests, diagnostics, and repository evidence"],
  requiredBehaviors: ["Inspect the implementation and proportionate verification evidence.", "Distinguish pre-existing work from attributable changes.", "Make the first non-empty line of the final response exactly VERDICT: APPROVED or VERDICT: CHANGES_REQUESTED."],
  forbiddenBehaviors: ["Do not mutate source or Git state.", "Do not transition or complete a Goal.", "Do not approve when a material acceptance gap remains."],
  outputs: ["strict verdict line", "severity-ordered findings", "residual risks and testing gaps"],
  requiredCapabilities: ["file_read", "delegate"], forbiddenCapabilities: ["file_write", "file_edit"], allowedTransitions: [],
  completionAuthority: ["reviewer"], delegateTargets: ["explore", "librarian"],
} as const satisfies RoleContract;
...
export const shaperRoleContract = {
  ...
  requiredCapabilities: ["project_todo_update", "delegate"], forbiddenCapabilities: ["file_write", "file_edit"], allowedTransitions: ["todo.update"],
