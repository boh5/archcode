...
3. Before advancing to critic_prd_review, check workflow_read for unresolved interactions. If any exist, resolve them via the delegate→propose→ask→resume loop before transitioning.
...
7. Before advancing to critic_spec_review, check workflow_read for unresolved interactions. If any exist, resolve them via the delegate→propose→ask→resume loop before transitioning.
...
9. Before advancing to awaiting_user_approval, check workflow_read for unresolved interactions. If any exist, resolve them via the delegate→propose→ask→resume loop before applying criticDecision "approved" and transitioning.
...
- Collect all proposals for the current gate, dedupe/merge related interactions, then call workflow_request_interactions once per gate; do not ask one question at a time when multiple proposals exist for the same gate.
...
- Batch related interactions and unknowns before asking the user; avoid serial one-question interruptions when the decisions are part of the same upgrade gate.`;
  ...
  test("criticDecision parameter instruction appears exactly once (not duplicated)", async () => {
    const result = await buildSystemPrompt(makeCtx({
      rolePrompt: orchestratorRolePrompt,
      allowedTools: workflowTools,
    }));

    const matches = result.match(/criticDecision parameter/g);
    expect(matches).not.toBeNull();
    expect(matches!.length).toBe(1);
  });
...
describe("clean-break critic prompt contract", () => {
  ...
  //   1. The prompt still mentions criticDecision as a valid parameter
  //   2. Critic outcomes are described as tool parameters, not Orchestrator decisions
  //
  // After T10:
  //   - criticDecision is removed from the orchestrator prompt
...
- Batch related interactions and unknowns before asking the user; avoid serial one-question interruptions when the decisions are part of the same upgrade gate.`;
  ...
  test("Orchestrator prompt does NOT mention criticDecision as a valid parameter", async () => {
    const result = await buildSystemPrompt(makeCtx({
      rolePrompt: criticContractRolePrompt,
      allowedTools: criticContractWorkflowTools,
    }));

    // Red phase: this FAILS because the prompt currently mentions criticDecision
    // in steps 5, 9, and 10 ("use workflow_update_stage with the criticDecision parameter").
    // After T10 removes it, this assertion will pass.
    expect(result).not.toContain("criticDecision");
  });
  ...
  test("Orchestrator prompt describes Critic outcomes as Orchestrator decisions, not tool parameters", async () => {
    ...
    // without mentioning criticDecision as a tool parameter.
    // Red phase: this FAILS because the prompt currently uses criticDecision as a parameter.
    expect(result).not.toContain("criticDecision");
  });
});
