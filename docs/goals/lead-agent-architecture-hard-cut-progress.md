...
- [x] Goal creation, continuation, and Analyst final-review gate
...
- Hard-cut `create_goal` to exact `{ objective }`. Replaced Reviewer completion with runtime-bound fresh direct `analyst + deep + goal-review` provenance and artifact-freshness checks.
...
- Goal creation confirmation belongs to Lead behavior through ordinary `ask_user`; Runtime does not interpret confirmation text.
...
- Added exact ordered tool-matrix tests for all five Agents and three full-runtime acceptance flows: ordinary Lead multi-child aggregation, Todo Discussion -> Ready -> fresh Lead, and Goal creation -> Build -> changes requested -> remediation -> fresh approval -> completion.
...
- Removed Runtime interpretation of Goal confirmation; Lead decides from conversational context whether to call `create_goal`.
...
- Goal confirmation uses ordinary `ask_user`; Lead chooses the wording and interprets the answer. `ask_user` and Runtime have no Goal-specific confirmation schema or behavior.
...
- The second independent review returned findings covering post-completion execution, Discussion override policy drift, non-mutating Build fixtures, and incomplete browser evidence. They were repaired and revalidated.
