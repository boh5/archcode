...
3. 其余本地工具和所有 MCP 工具通过一个 `tool_search` 按需检索并加载完整 schema。
...
- **一个搜索入口**：只有 `tool_search`，查询自然语言能力描述；名称精确/前缀只作为同一排序算法的加分项，不另做 `tool_get`、`list_all_tools` 或第二种调用协议。
...
- **硬切，不兜底**：删除当前 eager MCP 拼接和旧 `tools.tools` 合约。搜索空结果、MCP 暂不可用或 provider 不支持厂商原生 Tool Search 时，都不得回退为全量 schema。
...
`角色基础授权 + Execution overlay + worktree eligibility + live MCP snapshot -> Authorized Catalog -> Core + State + Execution 已加载项 -> ResolvedToolSet -> Provider`
...
每条内存索引只保存搜索需要的字段：规范工具名、namespace/server、description、参数名、参数说明和 enum literals，以及 descriptor digest。Digest 统一为稳定 key order 的 UTF-8 JSON 做 SHA-256，规范输入精确包含 `{sourceKind, namespace, registryName, description, aiJsonSchema, traits, outputPolicy}`；不包含 execute closure、MCP handle、secret、连接 epoch 或 provider wire。Catalog digest 是按 `namespace + registryName` 排序后的 `[registryName, descriptorDigest]` 再做同样 SHA-256。搜索采用一条确定性混合排序：

1. 对 query 与索引文本做大小写、分隔符、Unicode 空白规范化；
2. 用 BM25/词项重合找能力描述，用字符 trigram 容忍名称拼写和变体；
3. exact name、namespace/name prefix 在同一总分中加权；
4. 分数相同按 namespace + registry name 稳定排序。

`tool_search` 输入只保留 `query`、可选 `namespace` 和 `limit`；`limit` 默认 5、最大 5。模型边界把本轮 catalog digest 随 `tool_search` call 一起写入 Tool Batch。正常执行与冷恢复执行都先调用 `resolveLiveAuthorizedToolCatalog`；当前 digest 已变化时精确返回 `TOOL_SEARCH_CATALOG_CHANGED` 且不加载，相同才在这份刚解析的 catalog 上运行搜索。一次成功调用把前 1–5 个命中工具全部加载，并返回名称、namespace 和短摘要；完整 description/schema 只在下一模型调用出现。
...
### 4. MCP 和 namespace 提示

System Prompt 不列全量工具名或工具 description，只加入一个 bounded namespace 摘要：固定本地能力组、ready 的内置 MCP 说明、用户 MCP server id/display name、工具数，以及 omitted count。单个摘要最多 120 UTF-8 bytes，总输出最多 4 KiB；截断后 `tool_search` 仍搜索完整 Authorized Catalog。
...
5. **把 MCP 从 eager 改为 searchable**
   ...
   - 保留角色授权、hot apply、run-local descriptor、retire/epoch、redaction 和外部副作用语义；增加 bounded namespace prompt projection，不加入 tool-level 列表。
...
| 初始提示又被 namespace 撑大 | 不放工具级名称/description；每条 120 bytes、总计 4 KiB、带 omitted count，搜索仍覆盖完整 catalog |
...
- 对六种 Agent 的无状态 fixture，真实 `resolveModelBoundary().tools.toAITools()` 只包含锁定 Core（以及存在 deferred 候选时的 `tool_search`），不含任一 MCP 完整 schema。
- Goal、Todo、PDF attachment、output ref、各 child 状态和 worktree 状态逐项 fixture 只加入状态表指定且当前 Agent 已授权的工具；状态消失后的下一模型边界撤销该 State 提升，除非同一工具另由当前 Execution 的 loaded ref 合法保留。`compress` 不在 State 表中。
...
### AC-03：搜索算法可重复且能找回内置长尾

- 同一 catalog/query/namespace/limit 在重复运行、重启和 descriptor 枚举顺序变化后返回同一排序；exact/prefix 是加分项，不是唯一命中条件。
...
- `tool_search` 一次只加载 1–5 个已授权命中项；result 只返回选中项短摘要，下一模型 step 才出现完整 schema。
...
- 空结果精确返回 `TOOL_SEARCH_NO_MATCH`；不存在自动 load-all、按精确名字旁路、隐藏重试、provider fallback 或 eager fallback。
...
- 修改后默认无状态 Lead 的真实 initial tool wire（Core + `tool_search`，不含 namespace prompt）不超过 6,000 `o200k_base` tokens，且相对当前 11,845-token 本地基线下降至少 45%。
- 默认 ready `context7`/`exa` 不再增加首轮完整 tool wire；namespace prompt projection 始终不超过 4 KiB，且没有 tool-level name/description/schema 列表。
...
- `tool_search` 仅搜索当前 Agent/depth/Session 状态已授权 descriptors；未授权工具名称即使完全匹配也不出现在结果、日志或 namespace 摘要中。
...
- 当前 eager MCP append 路径、旧固定 tool-count 断言和所有被替换实现已删除；无 feature flag、旧行为 fallback、兼容 reader、deprecated export 或墓碑测试。
