...
- 删除固定“非简单任务启动 2-4 个研究 child、至少一个 Explore”的规则。是否委派和并行只由关键路径、依赖、ownership、上下文隔离和模型能力决定。
- `promptProfileId` 直接删除，替换为 model-level capability 数据和小型 overlay；禁止按 provider/model 复制完整 Prompt。
...
- 真实跨模型 live eval 不作为本 Goal 的 `DONE` 门槛；它是后续发布门槛。本 Goal 交付 capability renderer、确定性场景测试和可独立运行的 live-eval manifest/命令接口。
...
Model capabilities ───────────┼─> PromptContractCompiler ─> final system/developer context
...
2. Runtime Envelope：Agent/Session/父子/depth、可委派目标、Goal/Todo/review mode、owned scope、MCP readiness、model capabilities。
...
10. Model Overlay：只包含 capability 所需的格式、并行和结构化结果降级规则。
11. Environment：cwd、项目根、版本控制、时间等事实。
...
2. 从 Session store、Agent factory、Goal/Todo context、MCP status 和 ModelInfo 生成不可变 snapshot；固定 Execution 级来源，每次 model call 刷新生命周期状态，将身份与生命周期提醒从 user-message hook 移入权威 Prompt 层。
...
6. 在 model config 中增加必填、model-level capability：`multiToolCallEmission: "single" | "parallel"`、`structuredToolCalls: "strict" | "best_effort"`、`instructionTier: "compact" | "standard" | "rich"`。缺失配置启动失败；overlay 不读取 provider 名称。child runtime 并发只由现有 `childPolicy.maxConcurrent` 和 admission 决定，不由模型 capability 削弱。
7. 明确 Skill、Memory、Project Instructions、Tool Visibility 的 authority 包装；修正 Goal memory 等接口注释与真实注入不一致，但不扩张各领域功能。
8. 增加 Prompt trace、合法模式快照、冲突 lint、三 capability profile 的确定性场景测试，以及接收显式 model manifest 的 opt-in live-eval 命令；完成全仓验证和旧路径搜索审计。真实模型运行结果留给后续发布门槛。
...
- snapshot 明确包含真实 agent/session/root/parent/depth、allowed targets、completion authority、review mode、Goal/Todo identity、owned scope、MCP readiness 和 capability profile；不适用字段明确为 `none`，不得靠模型推断。
...
### AC-07：模型适配可组合、无厂商 Prompt 分叉

- 每个 model config 必须显式声明三项带锁定枚举的 capability；缺失或非法值启动失败，无 provider-name/model-name 推测 fallback。
- 最终 Prompt 只因 capability overlay 改变并行、结果提交和指令复杂度提示；Shared Kernel、Role Contract 和 Collaboration 语义保持同一来源。
- 仓库不存在 `if provider === ...`、按模型名称选择整份 Prompt、八角色 × 厂商的 Prompt 副本。
- `multiToolCallEmission=single` 只让 overlay 指示模型逐次发出 tool call，不降低 runtime child concurrency；后台 child 是否可重叠只由依赖、ownership 和 `childPolicy.maxConcurrent` 决定。
- `structuredToolCalls=best_effort` 时仍要求工具提交结果，但使用更短的原子说明和一次明确纠错机会，失败后进入 `CHILD_RESULT_REQUIRED`，绝不回退自由文本完成。

### AC-08：验证与真实回归完整

- Prompt snapshot、contract/lint、delegation admission、ownership、resume、child result、Goal Reviewer、MCP readiness 和三种 capability overlay 的确定性目标测试全部通过。
