...
> 本文覆盖 2026-09-03 对 ArchCode 最近 Todo、Session、Plan 和实际运行记录的综合审计。它取代此前只覆盖 Compression 与 Skill tools 的窄范围版本。实施完成不能以“代码已改”或“基本可用”判定；AC-01 至 AC-10 必须逐项提供决定性证据。
...
| I-02 | P0 | `skill_read` / `skill_list` 的模型可见说明和错误 hint 不足，模型反复构造 `/`、`:first`、`invalid`、`PLACEHOLDER` 等参数 | 首次调用与错误恢复都有可复制的正确 JSON，严格校验仍保留 |
...
| I-07 | P1 | HITL 暂停时模型说明藏在默认折叠 Work 内，Question/Permission 决策单又占据大量空间且整卡不可折叠 | 暂停 Work 默认可读，Work 与决策单可以独立折叠，正文空间和滚动位置稳定 |
| I-08 | P2 | 左侧没有 Running Todo 入口，仍处于 Idea 的 Discussion 正在运行时难以返回 | 导航显示由真实 Session 活动推导的 Running Todo，点击回到确定的运行 Session |
| I-09 | P2 | Todo display lead 与塑形模板冲突，列表出现重复的 `Outcome` 或以 localhost URL 为标题 | display lead 来自具体内容，新模板强制具体标题，结构标题和独立 URL 不再冒充标题 |
...
- `projectTodoDisplayLead()` 当前选择任意第一个 Markdown heading，而 builtin shaping template 第一项正是通用的 `## Outcome`。
...
- 执行时先 fetch，并以当时 canonical `origin/main` 的精确 SHA 创建独立 worktree；记录该 SHA、当前营销分支和未提交文件，但不得从 `codex/add-marketing-skills` 派生实现分支。若网络或 canonical ref 无法验证，停止实施并报告，不能猜 base。
- 记录安装版 `archcode` 的 `/api/bootstrap`；Plan 文档可以作为只读输入带入，但实现 diff 不得夹带当前 checkout 的无关变更。
- 把上述 Session 统计转成只读回归 fixture 或测试输入；测试不得依赖用户当前 `.archcode/runtime` 文件持续存在。
- 产出 issue-to-test trace matrix，I-01 至 I-09 每项必须对应 owner、实现步骤和 AC。
...
- 更新 `skill-read.ts`、`skill-list.ts` 和 `agents/model-tool-projection.ts` 的最终模型可见 Description/字段说明：
  - `skill_read({"name":"run-goal"})` 是入口首读；首次不得传 `resource`。
  - resource 只能复制入口 Resources 的精确相对路径，禁止 `/`、`SKILL.md` 和猜测路径。
  - `skill_list({})` 与 `skill_list({"agent_type":"build"})` 是首屏示例；首屏省略 cursor，后续只复制上一成功页的 `nextCursor`。
...
- 在 Current Context 增加完整 Session Todo 和 latest direct-child JSON；不新建 DTO、缓存、Runtime State manager 或第二套 Agent Tree。
...
- 覆盖实时暂停、刷新重进、Needs-you 深链、root/child owner、多 pending request、SSE 更新、用户手动 override、follow-latest、历史阅读锚点、键盘和窄屏。
...
- 增加中英文 Markdown、`## Outcome`、URL 后有说明、URL-only、placeholder-only、字段标签无正文、fenced code、无 heading、空内容和长 Unicode 的测试；不新增 title 字段、不迁移 schema、不为旧格式保留另一套 renderer。

### Step 8：集成、真实回归和文档收口

- 更新 `AGENTS.md` 中 Active Skill boundary、child completion、重复失败、Compression state projection 和 UI projection 的当前架构事实。
- 按依赖顺序完成 targeted tests，再执行 `bun run typecheck`、`bun run test`、`bun run build`、`git diff --check`。
...
- 从实际 `ResolvedToolSet.toAITools()` 检查 `skill_read` 正例、resource 顺序和禁止项；从 model-call-local projected descriptor 检查 `skill_list` 首屏/target/nextCursor 示例。
- `/` resource 的错误直接给出 `skill_read({"name":"run-goal"})`；猜测的缺失 resource 要求重读入口并复制 Resources，不建议创建文件。
...
- 实施发生在以执行时 `origin/main` 精确 SHA 为 base 的独立 worktree，最终 diff 不包含审计前的营销 Skill 或其他无关改动。
- 独立 Sol(max) Reviewer 检查 I-01 至 I-09、AC-01 至 AC-10、hard cut、owner 边界和过度设计；所有 blocking/high 与明显问题完成 fix -> review 闭环。
...
| Current Context 随 Todo/child 增长 | 保持 authority 完整与确定性 JSON；不暗中截断。容量优化需有新测量后单独设计 |
...
只有当 I-01 至 I-09 均映射到已完成实现，AC-01 至 AC-10 全部有决定性证据，独立最终 Review 无 blocking/high，且没有 fallback、旧兼容路径、墓碑测试、模型特判或额外框架时，本 Goal 才算完成。任何一项缺证据都必须报告为 `NOT_DONE`，不能用总体测试通过替代。
