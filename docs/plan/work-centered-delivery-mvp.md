...
ArchCode 当前是一个以 Session、Session Goal 和 Automation 为执行原语的资源中心型工作台。候选演进方向是以 Work 为中心：用户只需要带来一件要完成的工作，并始终停留在一条连贯的对话中，由 ArchCode 共同澄清、规划并交付结果。
...
用户看到的是一条连续的 Work 对话。运行时可以在其下使用多个相互隔离的 Agent Session，以及 root Session 上可选的 Goal。
...
- 返回之前的阶段；
- 用新的阶段运行取代旧版本；
- 选择普通 Session，或为 root Session 启用 Goal；
- 创建阶段内部步骤；
- 停止或取消整个 Work。
...
`- phaseRuns[]
   ...
   |- carrier: session | session_goal | background
   |- carrierId
...
| Tool | Agent 可以使用的能力 |
| Session / Session Goal | 执行载体；Goal 是 root Session 的持久自动续跑协议 |
...
User <-> Work Lead（现有 Engineer）
            ...
            |           `- 非简单工作使用带 Goal 的 Engineer -> Build -> 独立 Reviewer gate
...
- 非简单、长期运行、多 Agent、需要预算、需要恢复或独立审查的工作由启用了 Goal 的 root Engineer 负责。

目标、约束和验证期待写入 Goal objective。用户可直接编辑、暂停、恢复或清除；运行时在下一次模型调用前注入更新。Agent 不能静默改变目标。

### 5.5 Reflect

MVP 负责人：Work Lead，或一次尽力而为的后台结构化运行。

Reflect 不需要专用 Agent 身份或独立 Goal。它对比意图、决策、合同、交付证据和用户干预，可以提出新的 Work 或 Automation，但不能静默执行。
...
- 建议 Deliver 应使用普通 Session 还是为 root Session 启用 Goal；
- 不进入实现。
...
MVP-0 不引入 `work_manage` 工具或持久化 Work 状态机。Work Lead 首先应通过普通对话、Skills、现有 `todo_write`、自然语言触发的 `create_goal`、HITL 和 Session 事件表达建议。
...
Session、其 Goal、子 Agent、工具调用和原始证据仍然可以检查，但不再被设想为用户必须理解的多个并列顶层对象。

## 11. Goal 和 Automation

Goal 继续作为 root Session 的核心运行时协议，在候选模型中承担内部 Deliver 的持续执行。它不是独立导航资源。
...
- `Work` 最初只是 root Session、其 Goal、HITL、子 Session、证据和后续工作的用户体验投影；
- `Discover`、`Plan` 和 `Reflect` 是由 Skills 和 Prompt 表达的可选方法；
- `Deliver` 复用 root Session 的 Goal，而不是创建第二个执行对象；
- 在持久化需求被证明之前，阶段标签只是建议性的展示元数据；
- 验证期间继续保留 Session Goal 和 Automation，不增加新的执行对象；
- 不为阶段增加新的固定 Agent 身份。
...
5. 当合同已经就绪且确实需要长期执行时，获得用户确认后以 `create_goal` 为当前 root Session 启用 Goal；
6. 让现有 Engineer、Build、运行时 evaluator、独立 Reviewer gate、HITL 和证据链路完成交付；
...
1. 把 Work 表示为现有 root Session、其 Goal、子 Session 树、HITL 和事件的只读投影；
...
6. 为已经批准的非简单合同在当前 root Session 上启用 Deliver Goal；
7. 把 Goal 执行和提问投影回 Work 对话；
8. 交付后生成尽力而为的 Reflect 回执；
9. 在保留当前 Session 和 Automation 导航的前提下，通过功能开关暴露 Work 投影。
...
- Skill 市场；
- 投影证明有用之前的全产品存储迁移；
- 删除 Session Goal 或 Automation 的内部实现；
...
1. 与显式选择普通 Session 或 Session Goal 相比，用户更喜欢从一条 Work 对话开始。
2. Discover 能改善合同质量，同时不会带来无法接受的流程负担。
3. 系统能够为简单工作正确跳过 Discover 或 Plan。
4. 自动从 Plan 交接到当前 root Session 的 Goal 能减少重复输入和模式管理。
5. 与分散的 Session、Session Goal 状态和 Automation 列表相比，Work 级聚合更容易理解。
...
下一步应该是在当前 Session 及其 Goal 体验内进行人工辅助实验，不增加 Work 投影，也不改变存储，只测试一条端到端路径：

```text
Discover Session -> Plan Session -> root Session Goal -> Reflect
```

只有真实使用证明这条路径改善了清晰度、完成率和信任，ArchCode 才应增加轻量 Work 投影。增加权威 Work 存储或泛化工作流模型，都需要第二道、更强的证据门槛。
