...
Reviewer 从 Phase 1 起就**强制**(orchestrator 不能跳过,`reviewed` 是 `done` 前置)。没有 Reviewer 的 Goal 退化为 Session(用 Session 即可)。Goal 核心价值就是 locked 验收条件 + 独立 Reviewer 验证。
...
Workflow 的 `artifact_read`/`artifact_write` 语义已删除。Phase 2 使用 Goal-scoped canonical artifact manager 和四个 Goal 工具:`goal_manage`(生命周期动作)、`Reviewer evidence flow`(Reviewer 机检证据)、`Goal session evidence read access`、`Goal session evidence write access`。artifact 只接受当前 Markdown 文件名:`planning notes`,`build notes`,`review notes`,`spec compliance notes`,`approval notes`,`budget notes`,`retry notes`,`final report notes`。
...
- orchestrator 调 `goal_manage` 工具,`action="create"` 创建 draft Goal(title/acceptanceCriteria/retryPolicy/approvalPoints/author)
- Goal state 写到 `.archcode/goals/{goalId}/goal.json`
- orchestrator 调 `goal_manage` 工具,`action="lock"` 锁定 draft Goal;`action="start"` 开始执行;`action="advance lifecycle state"` 推进 plan→build→review;`action="retry"` 重试失败 Goal
- Reviewer 调 `Reviewer evidence flow` 工具,`action="check_done"` 记录 验收条件机检证据;调 `goal_manage` 工具,`action="finalize_review"` 给出 DONE/NOT_DONE 最终判定
- Plan/Build 只拥有 `Goal session evidence read access`/`Goal session evidence write access`,不拥有生命周期/证据工具
...
- `goal_manage` 生命周期动作工具(create/lock/start/advance lifecycle state/retry/finalize_review) + `Reviewer evidence flow` Reviewer 证据工具(action="check_done")
