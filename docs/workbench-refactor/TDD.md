...
**状态**:技术设计,描述完整实现方案。实施按 5 阶段(先 Goal 后 Loop)分步,由用户根据 PRD 拆任务执行。
...
| 失败模式 | 不知道发生了什么 | **完整 session 树 + 审计日志 + 回滚** |
...
| `review` | 架构师批量审查产物 | verdict 是 approve/reject/request_changes,驱动 Goal 状态机 | **新能力**,现有代码无对应 |
...
**HITL 持久化:MVP in-memory only**。pending HITL 不持久化到磁盘。Server restart 后:
1. pending HITL 丢失(in-memory)
2. 关联的 Goal 标 `paused`(或 `escalated`,由 retry policy 决定)
3. 用户手动 resume(创建新 session,fresh context)

不引入磁盘持久化的复杂度。后续如需持久化,再加 `.archcode/hitl/{hitlId}.json`。

`PermissionService` / `AskUserService` 已是 session-scoped deferred Promise + SSE 推送 + 跨网络恢复。`HitlService` 复用这个模式:
- `HitlService.request(sessionId, kind, payload, trigger)` → 推 `hitl.request` 到 session ring → 返回 Promise
- `POST /api/hitl/:id/respond` → resolve Promise
- Abort/cleanup 安全 resolve pending(超时 approval,cancelled review)
- `listPending(projectSlug?, goalId?, loopId?)` 支持按项目/Goal/Loop 过滤
...
Workflow 的 `artifact_write`(atomic write + path containment)能力保留,但**不再作为独立工具**。Build agent 用 `file_write` 写文件,Goal 的 `file_exists` Done 条件检查产物。如需 path containment,在 `file_write` guard 里保留 workspace 边界检查(已有)。
...
**注意**:`agents` 用 `z.strictObject` 强制 6 个 agent 全部必填,不是 `z.record` + enum(避免漏配)。未配置的 agent 实例化时 fail-fast,与现有"missing agent model config fails fast"一致。**无 `goals?` / `loops?` 节**——Goal/Loop 运行时创建,不进 config(见 §7.2.1)。
...
// deny→工具不执行 和 approve_always→持久化 是 permission 模块根据返回值做的,不是 HITL 的职责。
  ...
  // review:架构师批量审查产物。verdict 驱动 Goal 状态机(approve→done, reject→retry, request_changes→build 修改)。
...
复用现有 `PermissionService` / `AskUserService` 的 deferred Promise + SSE 推送 + 超时/abort 安全 resolve 模式。**MVP in-memory only**:pending HITL 不持久化,server restart 后 pending HITL 丢失,关联 Goal 标 `paused`/`escalated`,用户手动 resume。
...
- 有 pending HITL 的 Goal → 标 `paused`(HITL in-memory,restart 后丢失)
...
```
(注:HITL MVP in-memory,无 `.archcode/hitl/` 目录。如后续加持久化,再加。)
...
- **API**:`POST /api/loops/:id/kill` → 设 killSwitch + abort current run;`POST /api/loops/kill-all` → 全局 kill
...
|---|---|---|---|
| **GitHub** | PR 列表 / PR diff / issue 列表 / open PR review comments / post PR comment / 开 PR | pr_babysitter / issue_triage / post_merge_cleanup | 4 |
| **CI** | 最近 CI run 状态 / 失败日志摘要 | ci_sweeper / dependency_sweeper | 4 |
...
- Config 在 `.archcode.json` 加可选 `integrations` 节(Strict Zod):
  ```json
  "integrations": {
    "github": { "token": "${GITHUB_TOKEN}", "owner": "archcode", "repo": "archcode" },
    "ci": { "provider": "github_actions" }
  }
  ```
- Connector 暴露 `listItems() / getItemDetail() / postComment() / createPR()` 等方法给 Loop triage 阶段调用
- **AI 自治边界**(见 §9.11):connector 只读 + 开 PR,不开 PR review 批准,不 merge

**v1 范围**:只接 GitHub(github.com,不含 GitHub Enterprise)+ GitHub Actions CI。GitLab / Bitbucket / CircleCI / Jenkins 留后续。
...
| 批准 / merge PR | ❌ | ✅ | 永远不允许 AI merge——Reviewer 存在的核心理由 |
...
- 每条 HITL 卡片显示:kind icon / 来源 session / Goal title / prompt / payload
...
按 **MVP 思想**开发:每个阶段交付一个可用的用户价值。对齐 PRD §5 的 5 阶段。**不套用 loop-engineering L0-L3**(那是 Loop 成熟度模型,不是产品开发顺序)。Loop 阶段内部可借鉴 L1→L2→L3 作为 Loop 自己的成熟度参考。
...
- HITL **MVP in-memory only**(restart 后 pending 丢失,Goal 标 paused)
...
- retry 完整:maxRetries + backoff + escalate 策略可配
- `spec_compliance` Done 条件(Layer 2 AI 判断,可选)
- Goal 详情 tabs 完整:Plan/Build/Review 结构化产物视图(不只 chat 流)
- Approval Queue 完整:全局 Dashboard + 就地,操作同步,跨项目汇总
- Goal budget 基础:per-Goal token 上限 + 超限暂停
- memory 接通 Goal:跨 session 记忆提取/注入
...
- L1→L2 毕业条件(L1 跑 2 周 noise <20% + readiness score ≥58 + verifier/worktree 在手动 fix 验证过 + denylist/测试命令已记录)
...
**5a 必需**(unattended Loop 基础,不做就不够"合上笔记本工作继续"):
...
- readiness score(Web UI badge,15 信号,0-100 分,loop-audit scoring 纯函数移植)
...
| **State rot** | S2 | crossRunState 写错 → 基于错状态跑 | state.md 人类可读 + git 可回滚 + `PATCH /state` API 手动 unstick |
...
| **Over-reach** | S2 | Loop 擅自做大改动 | L2 限定 minimal-fix pattern;大改动走 human_gate(approvalPoints) |
| **Parallel collision** | S3 | 多 loop 同分支并发改 | 跨 loop 协调:同分支每小时最多 1 loop + maxConcurrent + 优先级(L3) |
...
| `packages/agent-core/src/hitl/` | **新建** service(**不合并** PermissionService/AskUserService,新增独立 service,MVP in-memory only) |
...
| `packages/agent-core/src/projects/types.ts` | `ProjectContext` -workflow +`goals: GoalStateManager` +`loops: LoopStateManager` +`hitl: HitlService` |
...
| `apps/server/src/routes/hitl.ts` | **新建**(HITL 队列 + respond + cancel,支持 goalId/loopId 过滤) |
...
| `apps/web/src/components/features/` | +Goal/Loop/ApprovalQueue/Dashboard 组件集;-PipelineStepper -StateTab(workflow) |
...
- ❌ 不做 cost / 美元预算(tokens + iterations 足够,后续视需要再说)
