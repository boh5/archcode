...
Goal
 └─ main session (orchestrator,架构师对话在这里)
     ...
     ├─ child session: reviewer (read-only + lsp + bash readonly + git_diff + tests_run + goal_evidence + goal_artifact_read/write,独立 session store)
...
架构师在 Goal draft 阶段选择(AI 可生成),lock 后不可变。Reviewer 通过 `goal_evidence` 工具执行 Done 条件机检,并调用 `goal_manage.finalize_review` 给出最终 DONE/NOT_DONE 判定。
...
| **reviewer** | core | read-only + lsp + git_diff + grep + glob + `goal_evidence`(内部白名单执行 tests_pass/typecheck_pass/command_succeeds) + `goal_manage.finalize_review` + `goal_artifact_read`/`goal_artifact_write` | **独立**(不共享 build) | 默认拒绝,用证据判完成 |
...
- **能跑工具**:`lsp_diagnostics`/`grep`/`git_diff`/`glob` + `goal_evidence`(内部白名单执行 `tests_pass`/`typecheck_pass`/`command_succeeds`——Phase 1 不给通用 bash,只给 `goal_evidence` 工具,它内部按 DoneCondition kind 白名单执行) + `goal_manage.finalize_review` + `goal_artifact_read`/`goal_artifact_write`——这是**核心差异化**vs Claude Code `/goal` evaluator(不能跑工具,只能读对话)
...
- orchestrator 调 `goal_manage` 工具,`action="create"` 创建 draft Goal(title/doneConditions/retryPolicy/approvalPoints/author)
- Goal state 写到 `.archcode/goals/{goalId}/goal.json`
- orchestrator 调 `goal_manage` 工具,`action="lock"` 锁定 draft Goal;`action="start"` 开始执行;`action="advance_phase"` 推进 plan→build→review;`action="retry"` 重试失败 Goal
- Reviewer 调 `goal_evidence` 工具,`action="check_done"` 记录 Done 条件机检证据;调 `goal_manage` 工具,`action="finalize_review"` 给出 DONE/NOT_DONE 最终判定
- Plan/Build 只拥有 `goal_artifact_read`/`goal_artifact_write`,不拥有生命周期/证据工具
...
2. run: 创建 main session(orchestrator)→ orchestrator 自主分解:
   ...
   e. reviewer 跑 goal_evidence action="check_done" → doneResults 写入;全部条件满足后 reviewer 调 goal_manage.finalize_review 给出 DONE/NOT_DONE
...
- Reviewer 工具集(§5.1)不包含 `git commit`(只读 + lsp + goal_evidence + goal_artifact_read/write),所以 Reviewer 不会改代码也不会提交
...
- Reviewer 工具集:read-only + lsp + git_diff + grep + glob + `goal_evidence` + `goal_manage.finalize_review` + `goal_artifact_read`/`goal_artifact_write`
...
- `goal_manage` 生命周期动作工具(create/lock/start/advance_phase/retry/finalize_review) + `goal_evidence` Reviewer 证据工具(action="check_done")
...
- Goal 执行路径(§9.4):lock → phase=plan → approval_point(after_plan) → phase=build → phase=review → goal_evidence 逐条 check_done → goal_manage.finalize_review → retry(fresh session,phase 重置 plan)
...
- `spec_compliance` Done 条件由 Reviewer 通过 Reviewer-only `goal_evidence` 工具(action="check_done")记录逐 criterion 结构化证据;Reviewer 外部 outcome 只有 `DONE` / `NOT_DONE`,由 `goal_manage.finalize_review` 最终判定。
