...
**goal_lead 自主分解 + 充分 HITL**:goal_lead 自己决定怎么拆任务,但在 `after_plan` / `before_complete` 等关键 phase 转换点和架构师充分讨论。`on_destructive_op` 是 build phase 内的 tool guard(不是 phase 转换门)。不是全自动,也不是手把手。
