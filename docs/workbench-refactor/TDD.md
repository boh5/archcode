...
3. [Acceptance Criteria:混合两层](#3-done-conditions混合两层)
4. [HITL:横切关注点](#4-hitl横切关注点)
5. [7-agent 架构](#5-7-agent-架构2-principal--3-core--2-ancillary)
...
| "我们有 certified compaction" | "架构师的 验收条件能扛过 compaction" |
...
1. **机器可检 验收条件**:Reviewer 能跑 `bun test`/`grep`/`lsp_diagnostics`,不只是读对话。Claude Code `/goal` evaluator 官方文档:"It does not call tools, so it can only judge what Claude has already surfaced in the conversation."
...
| **Goal** | Session + (1) 机器可检 验收条件 (2) 独立 Reviewer (3) 失败换新上下文重试 | 验收条件**运行前锁定、运行中不可变** + Reviewer ≠ Implementer | 验收条件机检全 true,或重试上限耗尽 |
...
1. **验收条件锁死**:谁写不重要,**何时锁**才重要。运行前锁定,运行中不可变。完成判定走机器可检,不走人类主观判断。
...
```
Goal
 └─ main session (goal_lead,架构师对话在这里)
     ├─ child session: plan (read-only + lsp + web_fetch)
     ├─ child session: build (read + write tools)
     ├─ child session: reviewer (read-only + lsp + bash readonly + git_diff + tests_run + Reviewer evidence flow + Goal session evidence access,独立 session store)
     ├─ child session: explore (read-only codebase grep)
     └─ child session: librarian (external docs/MCP)
```

- **main session**:goal_lead 拥有,架构师在这里对话。Chat tab = main session 对话视图。
...
例:每日 triage 报告用 Session 足够;每日依赖扫描+安全补丁用 Goal(`tests pass` + `command succeeds` 验收条件 + Reviewer 审)。
...
架构师在 Goal draft 阶段选择(AI 可生成),lock 后不可变。Reviewer 通过 `Reviewer evidence flow` 工具执行 验收条件机检,并调用 `goal_manage.finalize_review` 给出最终 DONE/NOT_DONE 判定。

| kind | params | 机检实现 | 来源 |
|---|---|---|---|
| `tests pass` | `{ command?: string }` | 跑 `bun test`(或指定 command),exit 0 | ArchCode 原生 |
| `typecheck passes` | `{ command?: string }` | 跑 `bun run typecheck`,exit 0 | ArchCode 原生 |
| `LSP diagnostics are clean` | `{ paths?: string[], severity?: "error" \| "warning" }` | `lsp_diagnostics` 无 error(或 warning) | ArchCode LSP |
| `file exists` | `{ path: string }` | 工作区内文件存在 | Castor `files_exist` |
| `grep contains` | `{ pattern: string, path?: string, minMatches?: number }` | ripgrep 匹配 ≥ minMatches | Castor `regex_in_file` |
| `grep has no matches` | `{ pattern: string, path?: string }` | ripgrep 无匹配(如"无 TODO""无 console.log") | ArchCode 扩展 |
| `command succeeds` | `{ command: string, timeoutMs?: number }` | 任意 shell 命令 exit 0(**通用 fallback**) | Castor `shell_returns_zero` |
| `user confirmation` | `{ prompt: string }` | HITL `kind="approval"`,架构师确认(**唯一非机器 check**) | ArchCode HITL |

**`command succeeds` 是万能逃生口**:任何无法用前 7 种表达的条件,都能用一条 shell 命令表达(如"curl health check 返回 200"、"mypy 无 error"、"git status 干净")。

### 3.3 Layer 2:AI 判断(可选,1 种 kind)

| kind | params | 执行 | 来源 |
|---|---|---|---|
| `spec compliance` | `{ specPath: string, focusAreas?: string[] }` | Reviewer 读 spec + 代码 + 跑工具,判断实现是否匹配 spec | Factory.ai Review Droid / Augment Intent Reviewer |
...
- `approvalPoints` 是 **phase 转换门**:Goal runner 在 phase 切换时检查 → 命中则调 HITL `approval` → 等用户决策 → 放行或拒绝 phase 切换
  ...
  - 单次工具调用另走 **tool permission**(不是 approvalPoint,见 §4.3)
...
- **`reviewed` 是 `done` 的前置**:Goal state machine 强制 Reviewer 通过才能进 `verifying`。Goal Lead 不能跳过 Reviewer。
- **lock 不可逆**:draft → locked 是单向。锁定后 验收条件不可改,防止 agent 运行中偷改完成定义。
- **fresh-context retry**:每次 retry 创建新 session(不继承上次失败的上下文),避免失败上下文污染。retry 时 phase 重置为 `plan`。

### 3.6 Reviewer day-one 强制

Reviewer 从 Phase 1 起就**强制**(goal_lead 不能跳过,`reviewed` 是 `done` 前置)。没有 Reviewer 的 Goal 退化为 Session(用 Session 即可)。Goal 核心价值就是 locked 验收条件 + 独立 Reviewer 验证。
...
HITL 是**与 Goal 平级的 peer service**,不是 Goal 的子模块。Goal 的 Reviewer validation flow 依赖 HITL 回答 `user confirmation` kind。HITL 可在任意 session(主或子)弹出,统一队列。
...
- Bash 默认允许；未知命令、一般删除以及普通 Git push/merge/rebase/reset 不因类别本身触发审批。
- Ask 只覆盖明确字面量越界/凭证、提权和固定系统修改/凭证外传形状；deny 只覆盖 ArchCode 控制面与可机械确认的灾难操作。
- 有限静态分析明确接受动态 shell、package script 和自定义二进制盲区；强隔离必须由部署环境提供。
...
**HITL 架构决策:不合并现有 service**。保留 `PermissionService`(tool permission 的 UX guardrail,不是安全边界)+ `AskUserService`(已有 ask_user)不动。新增 `HitlService` 只管 Goal approvalPoints 和 review kind 的 queue + respond + cancel。`HitlService` 复用现有 deferred Promise + SSE 推送 + 超时/abort 安全 resolve 模式,但是独立 service。
...
## 5. 7-agent 架构(2 principal + 3 core + 2 ancillary)

### 5.1 全景

| 角色 | 类型 | 工具集 | session store | 用途 |
|---|---|---|---|---|
| **engineer** | principal | 完整工程工具 + `goal_create` + delegation + MCP | 自己的 | 普通 Session 中直接工作,必要时委派 |
| **goal_lead** | principal | 只读协调工具 + `goal_manage` + delegation + MCP | 自己的 | 推进已有 Goal,委派实现和验证 |
| **plan** | core | read-only + lsp + web_fetch + grep + glob | 独立 | 想清楚要做什么 |
| **build** | core | read + write + edit + bash + lsp + ast_grep_replace | 独立 | 实现代码 |
| **reviewer** | core | read-only + lsp + git_diff + grep + glob + `Reviewer evidence flow`(内部白名单执行 tests pass/typecheck passes/command succeeds) + `goal_manage.finalize_review` + `Goal session evidence read access`/`Goal session evidence write access` | **独立**(不共享 build) | 默认拒绝,用证据判完成 |
...
**工具集在 agent definition 里硬编码**——principal Agent **不能**在 delegation 时修改子 agent 的工具集。这是 Workflow 原有的 tool permission boundary,迁移到 agent definition 层。这是**安全边界,不是装饰**。

### 5.3 Reviewer 的核心差异化

- **强制**:Goal Lead 不能跳过(`reviewed` 是 `done` 前置)
- **独立 session store**:不共享 Build 的 store,避免被 Build 的叙事带偏
- **能跑工具**:`lsp_diagnostics`/`grep`/`git_diff`/`glob` + `Reviewer evidence flow`(内部白名单执行 `tests pass`/`typecheck passes`/`command succeeds`——Phase 1 不给通用 bash,只给 `Reviewer evidence flow` 工具,它内部按 acceptance criterion kind 白名单执行) + `goal_manage.finalize_review` + `Goal session evidence read access`/`Goal session evidence write access`——这是**核心差异化**vs Claude Code `/goal` evaluator(不能跑工具,只能读对话)
...
- goal_lead 通过 `delegate` 工具委派
- `delegate(agent_type, persona?, task, context)` → AgentFactory → ConfiguredAgent(硬编码工具集 + 独立 store)
  - `agent_type`:goal_lead/plan/build/reviewer/explore/librarian(决定工具集)
  - `persona`:可选,影响 system prompt 语气(如 "product manager"/"spec writer"/"critic"),不影响工具集
- 子 agent 完成后 reminder 回到 goal_lead
- 深度限制:goal_lead(maxDepth=3)→ plan/build/reviewer(maxDepth=2)→ explore/librarian(maxDepth=1)
...
- 新 agent 系统取代现有 goal_lead/explore
- UI 全量重设计
- Config schema 重设计
...
| Workflow 硬约束 | 迁移到 | 说明 |
|---|---|---|
| 1. Tool permissions per role | **agent definition 工具集硬编码** | plan/build/reviewer 各自工具集,goal_lead 不能改 |
...
| 3. Artifact prerequisites | **Goal `acceptanceCriteria: [{kind:"file exists"}]`** | 架构师声明"完成时必须有 spec.md"= 验收条件 |
...
Workflow 的 `artifact_read`/`artifact_write` 语义已删除。Goal 不再维护专用 artifact/evidence 工具或 artifact 存储；证据直接来自普通 Session 输出、diff、工具结果、审批记录、预算/重试状态和 Reviewer summary。`goal_create` 只创建 draft，`goal_manage` 只处理已启动 Goal 的生命周期与 Reviewer finalization。
...
**注意**:`agents` 用 `z.strictObject` 强制 7 个 agent 全部必填,不是 `z.record` + enum(避免漏配)。未配置的 agent 实例化时 fail-fast,与现有"missing agent model config fails fast"一致。**无 `goals?` / `loops?` 节**——Goal/Loop 运行时创建,不进 config(见 §7.2.1)。Phase 2 也不新增 `~/.archcode/config.json` budget/retry defaults/schema 字段;这些值属于 Goal create input 和持久化 Goal state。
...
**Goal 保持同一模式**:
- 架构师在普通 Session 里说"我要做一个 Goal"
- Engineer 调 `goal_create` 创建 draft Goal(objective/acceptanceCriteria/useWorktree)
- Goal state 写到 `.archcode/goals/{goalId}/goal.json`
- Web/API Run 或 Loop Runner 确定性地启动 Goal 并创建专属 Goal Lead Session
- Goal Lead 通过 `goal_manage` 执行 block/resume/begin_review/retry/cancel，并将实现委派给 Build
- Reviewer 调 `goal_manage`, `action="finalize_review"` 给出 DONE/NOT_DONE 最终判定
- Plan/Build 不拥有 Goal 生命周期工具
...
3. 架构师通过 UI 管理 Loop 生命周期,不是改 `~/.archcode/config.json` 然后重启 server

**唯一在 config 里的是 agent 配置**——因为 model/variant/options 是启动时解析的,未配置的 agent 实例化时 fail-fast。这是现有模式的延续,不是新增。

### 7.3 Agent 配置迁移

现有 `~/.archcode/config.json` 的 `agents.goal_lead` / `agents.explore` 配置保留。新增必须配置:`agents.plan` / `agents.build` / `agents.reviewer` / `agents.librarian`。**未配置的 agent 实例化时 fail-fast**,与现有"missing agent model config fails fast"一致。
...
export type GoalStatus =
  | "draft" | "running" | "reviewed" | "completed" | "failed" | "escalated";
...
export interface GoalState {
  ...
  title: string;
  objective: string;
  acceptanceCriteria: string;  // natural-language criteria, fixed for the run
  status: GoalStatus;
  reviewerAgent: string;       // default "reviewer"
...
export type GoalStreamEvent =
  ...
  | { type: "goal.review_outcome"; goalId: string; outcome: ReviewerOutcome }
...
| `POST` | `/api/projects/:slug/goals/:id/lock` | **锁定 验收条件**(不可逆) |
...
5. run_kind:
   - runKind="session": 创建新 session → goal_lead 跑
...
```
1. lock: 验收条件锁定(draft → locked),记录 lockedBy
2. run: 创建 main session(goal_lead)→ goal_lead 自主分解:
   ...
   c. phase=build: delegate(build, "按 plan 实现") → build session
      - build phase 内 file/Bash 等工具按各自有限 permission 规则决定 allow/ask/deny，ask 时进入 HITL approval
   d. phase=review: delegate(reviewer, "审查完成度") → review session(独立 store)
   e. reviewer 跑 Reviewer evidence flow action="check_done" → reviewResults 写入;全部条件满足后 reviewer 调 goal_manage.finalize_review 给出 DONE/NOT_DONE
3. verify: 所有 acceptanceCriteria 机检
...
**goal_lead 自主分解 + 充分 HITL**:goal_lead 自己决定怎么拆任务,但在 `after_plan` / `before_complete` 等关键 phase 转换点和架构师充分讨论。单次工具调用另走 tool permission，不属于 phase 转换门。不是全自动,也不是手把手。
...
```
Goal evidence is ordinary session output, diffs, tool results, approval records, retry/budget metadata, and Reviewer summaries; no Goal-specific artifact version/revision/latest files are written. Goal memory is isolated from project memory at `.archcode/memory/`. HITL durable queue records redacted display payloads and project-scoped approval decisions.
...
- Config 在 `~/.archcode/config.json` 加可选 `integrations` 节(Strict Zod):
  ```json
  "integrations": {
    "github": {
      "enabled": true,
...
| `git push`(推到远端 branch) | 默认工作流不主动执行 | ✅ | 角色/工作流约束；Bash permission 对普通 `git push` 默认允许 |
| `git merge` / `git rebase` / `git reset --hard` | 默认工作流不主动执行 | ✅ | 角色/工作流约束，不依赖 Bash guard |
...
**实现**:
- Bash 权限保持窄策略：默认允许 Git 命令；只对 ArchCode 管理的 worktree/ref、root-wide 非 dry-run `git clean` 等控制面操作作确定性拒绝
- `git commit` 允许,但 commit message 里自动追加 `[archcode-auto]` 标记,便于事后过滤
- Reviewer 工具集(§5.1)不包含 `git commit`(只读 + lsp + Reviewer evidence flow + Goal session evidence access),所以 Reviewer 不会改代码也不会提交
...
**背景**:Factory Missions(2026-04)证明"有序 Goal 序列 + 阶段门控 + 总体验收"对超大功能开发有价值。ArchCode 当前用"单个 Goal + goal_lead 自主分解 plan→build→review"处理大功能。如果后续 dogfood 发现单个 Goal 上下文累积影响质量(Factory 已证明单 session 累积是死穴),再评估引入 Mission。
...
**当前替代方案**:用户手动开多个 Goal,在 Goal description 里写"这是 N 阶段大功能的第 K 阶段"。goal_lead 自主分解时知道全局上下文。不完美但够 MVP。
...
Projects
 └─ {project}
     ├─ Goals
     │   └─ {goal}
     │       ├─ Overview(acceptance criteria + 进度)
     │       ├─ Plan(plan session 视图)
     │       ├─ Build(build session 视图)
     │       ├─ Review(review session + reviewResults)
...
| Tab | 内容 |
|---|---|
| Overview | 验收条件列表 + 各条件机检结果(✅/❌)+ 进度条 + retry 链 |
| Plan | plan session 的对话/工具调用视图(只读回放) |
| Build | build session 的对话/工具调用/diff 视图 |
| Review | review session + reviewResults + Reviewer 证据 |
| Chat | **main session 对话视图**(架构师与 goal_lead 对话) |
| Sessions | 子 session 列表 + retry 链 + 委派树 |
| Evidence | 普通 session 输出、diff、工具结果、审批与 Reviewer summaries(只读回放) |
...
- Protocol 类型定义(§8):GoalState(含 phase/author/lockedBy)/HitlRequest/acceptance criterion(discriminated union)/SSE 事件(LoopState 暂不定义)
- Config schema(§7):`agents` 用 `z.strictObject` 强制 7 agent(engineer/goal_lead/plan/build/reviewer/explore/librarian),无 `goals?`/`loops?` 节
...
- Reviewer 工具集:read-only + lsp + git_diff + grep + glob + `Reviewer evidence flow` + `goal_manage.finalize_review` + `Goal session evidence read access`/`Goal session evidence write access`
...
- AI 可生成 验收条件(含 command succeeds),用户 lock 确认后生效;goal.json 记 author + lockedBy
- Goal 状态机:draft→locked→running→verifying→reviewed→completed/failed/escalated + 显式 phases(plan→build→review)
- Goal 执行路径(§9.4):lock → phase=plan → approval_point(after_plan) → phase=build → phase=review → Reviewer evidence flow 逐条 check_done → goal_manage.finalize_review → retry(fresh session,phase 重置 plan)
- approvalPoints:`after_plan` / `before_complete`(phase 转换门);单次工具调用另走 tool permission
...
- `spec compliance` 验收条件由 Reviewer 通过 Reviewer-only `Reviewer evidence flow` 工具(action="check_done")记录逐 criterion 结构化证据;Reviewer 外部 outcome 只有 `DONE` / `NOT_DONE`,由 `goal_manage.finalize_review` 最终判定。
- `NOT_DONE` 写结构化 Operator repair context,不保存 raw Reviewer/LLM 输出或 chain-of-thought;DONE 走 reviewed/completed 路径。
- Goal 详情 tabs 完整:Plan/Build/Review/Spec/Budget/Approvals/Retry/Final Report 汇总普通 session evidence,不只 chat 流。
- Evidence 来自普通 session 输出、diff、工具结果、审批记录、预算/重试状态和 Reviewer summaries;无 Goal-specific artifact version/revision/latest 文件。
...
- `~/.archcode/config.json` 不新增 Phase 2 `goals`、budget 或 retry defaults/schema 字段;budget/retry 是 Goal 创建输入和持久化 Goal state。
...
- 自定义 tool profiles 和 auto-approval 模式。硬性的工具可用范围继续由 agent definition 与固定 Loop tool profile allowlist 承担；permission/HITL 只提供面向明显误操作的交互护栏
...
| **Compaction 漏掉 验收条件** | S2 | 长跑 Goal,compaction 把 验收条件摘要丢了 | 验收条件在 `goal.json` 持久化,不在对话上下文里——compaction 不影响 |
...
| 文件 / 目录 | 变更 |
|---|---|
| `packages/protocol/src/types.ts` | +GoalState(含 phase/author/lockedBy) +LoopState +acceptance criterion(discriminated union) +ScheduleSpec +BudgetSpec +HitlRequest +SSE 事件 |
| `packages/agent-core/src/config/schema.ts` | `agents` 改 `z.strictObject` 强制 7 agent(engineer/goal_lead/plan/build/reviewer/explore/librarian),无 `goals?`/`loops?` 节(运行时创建) |
| `packages/agent-core/src/goals/` | **新建** types/state/runner/Reviewer validation flow/retry |
...
| `packages/agent-core/src/agents/definitions/{plan,build,reviewer,librarian}.ts` | **新建 4 个 agent 定义**(goal_lead/explore 保留改造) |
...
| `apps/web/src/components/features/` | +Goal/Loop/ApprovalQueue/Dashboard/Goal evidence summaries 组件集;-PipelineStepper -StateTab(workflow) |
| `README.md` | config 文档同步(AGENTS.md 约定):7 agent 必填,无 goals/loops 节 |
| `AGENTS.md` | 更新 agent 架构表(7 agent)、工具表(删 workflow 工具,加 goal/hitl/loop 工具)、目录结构 |
...
|---|---|---|
| 6 primitives + memory | §2 三层原语 + 现有 memory system | Automations→Schedule, Worktrees→(L3 可选), Skills→AGENTS.md+memory, Connectors→MCP, Sub-agents→7-agent, Goals→Goal 原语(loops 发现持续工作,goals 完成有界任务), State→crossRunState |
...
| loop-budget notes per-loop 限制 | `BudgetSpec` + on-exceed protocol(pause + log + escalate) | 代码强制,不是文档约定 |
...
| denylist for auto-merge paths | 不采用路径名驱动的 Bash 审批；发布/合并仍由各自工作流显式控制 | 避免把业务目录名误当安全边界 |
...
| Tool-specific starters(grok/claude-code/codex) | ArchCode 是 tool-agnostic,model 在 `~/.archcode/config.json` |
