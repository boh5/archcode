...
describe("server lifecycle", () => {
  test("setupGracefulShutdown registers signal handlers", () => {
    ...
    const handle = setupGracefulShutdown(server, target, { process: processRef });
  ...
  test("shutdown delegates to the Host target, stops, then exits", async () => {
    ...
    const handle = setupGracefulShutdown(server, target, { process: processRef, log: () => undefined });
    expect(handlers.has("SIGTERM")).toBe(true);
  ...
  test("shutdown exits with code 1 when running jobs exceed timeout", async () => {
    ...
    const handle = setupGracefulShutdown(server, target, { process: processRef, timeoutMs: 1, log: () => undefined, error });
    expect(handlers.has("SIGINT")).toBe(true);
