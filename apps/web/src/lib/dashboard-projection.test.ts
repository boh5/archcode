...
function read(
  overrides: Partial<DashboardReadProjection> = {},
): DashboardReadProjection {
  return { sessions: [], automations: [], errors: [], ...overrides };
}

function session(
  overrides: Partial<DashboardRootSession> = {},
): DashboardRootSession {
  return {
    projectSlug: "alpha",
    projectName: "Alpha",
    rootSessionId: "root",
    sessionTitle: "Root work",
    createdAt: 1,
    updatedAt: 1_000,
    ...overrides,
  };
...
function automation(
  overrides: Partial<DashboardAutomation> = {},
): DashboardAutomation {
  return {
    projectSlug: "alpha",
    projectName: "Alpha",
...
function hitl(
  overrides: Omit<Partial<ScopedHitlView>, "view"> & {
    view?: Partial<HitlView>;
  } = {},
): ScopedHitlView {
  ...
  return {
    ...
    view: {
      hitlId: "ask",
      owner: { type: "session", id: "child" },
      source: { type: "ask_user", toolCallId: "tool" },
      status: "pending",
...
describe("deriveDashboardSections", () => {
  test("keeps every attention item but removes its family from lower sections", () => {
    const result = deriveDashboardSections({
      read: read({
        sessions: [
          ...
          session({
            rootSessionId: "available",
            sessionTitle: "Continue",
            updatedAt: 2_000,
          }),
        ],
      ...
      hitl: [
        ...
        hitl({
          view: {
            ...
            displayPayload: { title: "Inspect", redacted: true },
          },
        }),
      ],
      activityFor: (_project, root) =>
        root === "available" ? "running" : "idle",
    });
    ...
    expect(result.running.map((item) => item.rootSessionId)).toEqual([
      "available",
    ]);
  ...
  test("applies section limits only after mutual exclusion and stable ordering", () => {
    ...
    const automations = Array.from({ length: 12 }, (_, index) =>
      automation({
        id: `auto-${index}`,
    ...
    const result = deriveDashboardSections({
      read: read({ sessions, automations }),
      hitl: [],
      activityFor: () => "idle",
    });
    ...
    expect(result.upcoming.map((item) => item.automationId)).not.toContain(
      "failed",
    );
    expect(result.attention.map((item) => item.identity)).toEqual([
      "automation:alpha:failed:i",
    ]);
  });
        ...
        hitl: [
          hitl({
            view: {
              ...
              displayPayload: { title: "Bad", redacted: true },
            },
          }),
        ],
        activityFor: () => "idle",
  ...
  test("removes recovered Goal and Session failures from attention on the next read", () => {
    ...
    expect(blocked.attention.map((item) => item.kind)).toEqual([
      "goal",
      "session_failure",
    ]);
    ...
    expect(recovered.running.map((item) => item.rootSessionId)).toEqual([
      "root",
    ]);
  });
  ...
  test("treats budget limits and timed out work as attention until their latest state recovers", () => {
    ...
    expect(limited.attention.map((item) => item.kind)).toEqual([
      "goal",
      "session_failure",
    ]);
    ...
    expect(resumed.continueWorking.map((item) => item.rootSessionId)).toEqual([
      "root",
    ]);
  });
  ...
  test("keeps all four sections mutually exclusive while preserving each section's ordering", () => {
    ...
    expect(result.running.map((item) => item.activity)).toEqual([
      "running",
      "stopping",
    ]);
    expect(result.continueWorking.map((item) => item.rootSessionId)).toEqual([
      "newer-idle",
      "older-idle",
    ]);
    expect(
      result.continueWorking.every((item) => item.activity === "idle"),
    ).toBe(true);
    expect(result.upcoming.map((item) => item.automationId)).toEqual([
      "earlier",
      "later",
    ]);
    ...
    const sectionOwners = [
      ...
      ...result.continueWorking.map(
        (item) => `continue:${item.sectionOwnerKey}`,
      ),
      ...result.upcoming.map((item) => `upcoming:${item.sectionOwnerKey}`),
    ];
    expect(sectionOwners).not.toContain(
      "running:session-family:alpha:attention",
    );
    expect(sectionOwners).not.toContain(
      "continue:session-family:alpha:attention",
    );
    expect(sectionOwners).not.toContain("upcoming:automation:alpha:failed");
  });

  test("keeps suspended and resuming families in the active section", () => {
    const result = deriveDashboardSections({
      read: read({
        sessions: [
          session({ rootSessionId: "waiting", updatedAt: 300 }),
          session({ rootSessionId: "resuming", updatedAt: 200 }),
          session({ rootSessionId: "idle", updatedAt: 100 }),
        ],
      }),
      hitl: [],
      activityFor: (_project, rootSessionId) => {
        if (rootSessionId === "waiting") return "waiting_for_human";
        if (rootSessionId === "resuming") return "resuming";
        return "idle";
      },
    });

    expect(result.running.map((item) => [item.rootSessionId, item.activity])).toEqual([
      ["resuming", "resuming"],
      ["waiting", "waiting_for_human"],
    ]);
    expect(result.continueWorking.map((item) => item.rootSessionId)).toEqual(["idle"]);
  });
});
