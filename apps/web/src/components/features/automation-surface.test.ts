...
describe("Automation navigation and detail actions", () => {
  test("detail exposes only Automation controls and Session-linked invocation history", async () => {
    ...
    expect(dialog).toContain('title="Definition controls"');
    expect(dialog).toContain('automation.status === "paused" ? "Resume" : "Pause"');
    expect(dialog).toContain("Delete Automation");
    expect(detail).toContain('title="Recent runs"');
  ...
  test("Automation creation uses the direct structured API", async () => {
    ...
    expect(dialog).toContain("Binding: Lead + principal");
  ...
  test("Automation enablement uses static domain status glyphs", async () => {
    ...
    const detail = await source("routes/automation-detail.tsx");
    expect(list).toContain('"border-text-tertiary bg-transparent"');
    expect(list).not.toContain("automationVisualKind(automation.status)");
    expect(detail).toContain("presentation.statusLabel");
  });

  test("detail header uses the locked control language", async () => {
    const detail = await source("routes/automation-detail.tsx");
    expect(detail).toContain('text-[21px]');
    expect(detail).toContain('min-[761px]:text-[26px]');
  ...
  test("detail keeps schedule and due metadata readable", async () => {
    const detail = await source("routes/automation-detail.tsx");
    expect(detail).toContain('title="Schedule & execution"');
    expect(detail).toContain('title="Message"');
