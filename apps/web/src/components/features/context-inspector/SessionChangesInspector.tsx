      ...
      <div className="space-y-1">
        {files.map((file) => (
          <button
            key={file.path}
            type="button"
            className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-left text-xs text-text-secondary hover:bg-bg-hover hover:text-text-primary"
          ...
          >
            ...
            <span className="shrink-0 text-[10px] text-text-muted">{file.status ?? "modified"}</span>
          </button>
        ))}
      </div>
      <button
