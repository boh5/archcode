...
const secondaryButtonClass = "inline-flex min-h-11 items-center justify-center gap-2 rounded-sm bg-bg-active px-3 text-[12px] font-medium text-text-secondary transition-colors duration-[var(--motion-fast)] hover:bg-bg-hover hover:text-text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand disabled:cursor-not-allowed disabled:opacity-40 sm:min-h-0 sm:h-8";
const dangerButtonClass = "inline-flex min-h-11 items-center justify-center gap-2 rounded-sm px-2.5 text-[12px] font-medium text-error transition-colors duration-[var(--motion-fast)] hover:bg-error-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand disabled:cursor-not-allowed disabled:opacity-40 sm:min-h-0 sm:h-7";
const selectClass = "h-9 w-full rounded-sm border border-border-control bg-bg-base px-3 text-[12px] text-text-primary outline-none transition-colors duration-[var(--motion-fast)] hover:border-text-secondary focus:border-brand focus:ring-2 focus:ring-brand-subtle sm:h-8";
...
function SettingsToggle({ checked, onChange, label, description }: { checked: boolean; onChange: (checked: boolean) => void; label: string; description: string }) {
  return <label className="flex min-h-11 cursor-pointer items-start gap-3 rounded-sm border border-border-subtle bg-bg-elevated px-3 py-3 transition-colors duration-[var(--motion-fast)] hover:border-border-default sm:min-h-0">
...
function ProjectMemoryEditor({ topics, topicCount, snapshot, selectedTopicName, topicDraft, loadingTopic, saving, onSelect, onNew, onDraftChange, onSave, onDelete }: { topics: MemoryTopicSummary[]; topicCount: number; snapshot: MemorySnapshot; selectedTopicName?: string; topicDraft?: TopicDraft; loadingTopic: boolean; saving: boolean; onSelect: (topic: MemoryTopicSummary) => void; onNew: () => void; onDraftChange: (draft: TopicDraft | undefined) => void; onSave: () => void; onDelete: () => void }) {
  ...
  return <section className="space-y-3 rounded-md border border-border-default bg-bg-surface p-4" aria-labelledby="project-memory-heading">
    ...
    <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(180px,0.8fr)_minmax(0,1.5fr)]">
      <div className="min-w-0 overflow-hidden rounded-sm border border-border-subtle bg-bg-base"><div className="border-b border-border-subtle px-3 py-2"><p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-text-tertiary">Topics</p></div>{topics.length === 0 ? <p className="px-3 py-4 text-[12px] text-text-tertiary">No project topics yet.</p> : <div className="max-h-72 overflow-y-auto">{topics.map((topic) => <button key={topic.name} type="button" onClick={() => onSelect(topic)} aria-current={selectedTopicName === topic.name ? "true" : undefined} className={`flex min-h-11 w-full items-start justify-between gap-2 border-b border-border-subtle px-3 py-2.5 text-left transition-colors duration-[var(--motion-fast)] last:border-b-0 hover:bg-bg-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand sm:min-h-0 ${selectedTopicName === topic.name ? "bg-selection-field" : ""}`}><span className="min-w-0"><span className="block truncate text-[12px] font-medium text-text-primary">{topic.name}</span><span className="mt-0.5 block truncate text-[11px] text-text-tertiary">{topic.description || topic.type || "Topic"}</span></span><span className="shrink-0 font-mono text-[10px] text-text-tertiary">{formatBytes(topic.capacity.bytes)}</span></button>)}</div>}</div>
