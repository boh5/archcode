...
export function ToolRunCard({
  ...
  const representative = running ? tools.at(-1) : tools[0];
  const settled = representative && isSettled(representative) ? representative.result : undefined;
  const isUnknownResult = settled?.details?.unknownResult === true;
  const visualKind = running
    ? "loading"
    : representative
      ? toolVisualKind(representative.state, isUnknownResult)
      : "unknown";
  ...
  const statusLabel = running
    ...
    : isUnknownResult
      ? "Unknown"
      : STATUS_LABEL[representative.state];
  ...
  return (
    <div className="max-w-[740px] shrink-0" data-testid="tool-run-card">
      ...
      {expanded && (
        ...
        >
          {items.map((item) => item.part.type === "reasoning"
            ? <ReasoningBlock key={item.part.id} part={item.part} />
