  ...
  return (
    <div className="relative flex h-full min-w-0 overflow-hidden bg-bg-base">
      ...
      <div className="relative min-h-0 min-w-0 flex-1 overflow-hidden">
        {savedConfirmation ? <p role="status" aria-live="polite" className="absolute right-3 top-3 z-40 rounded-[var(--shape-card)] border border-success/25 bg-success-muted px-3 py-2 text-[11px] text-success shadow-sm">Todo saved</p> : null}
...
function InspectorOverlay({ kind, open, topInset, width, returnFocusRef, onClose }: { kind: "session"; open: boolean; topInset: SessionInspectorTopInset; width: number; returnFocusRef: RefObject<HTMLElement | null>; onClose: () => void }) {
  return <DialogPrimitive.Root open={open} onOpenChange={(nextOpen) => { if (!nextOpen) onClose(); }}><DialogPrimitive.Portal><DialogPrimitive.Overlay className="fixed bottom-0 left-12 right-0 z-[60] bg-black/55 min-[761px]:left-[52px]" style={{ top: topInset }} /><DialogPrimitive.Content aria-describedby={undefined} data-inspector-placement="overlay" className="fixed bottom-0 right-0 z-[65] overflow-hidden border-l border-border-default bg-bg-surface shadow-lg outline-none" style={{ top: topInset, width, maxWidth: "calc(100vw - 48px)" }} onCloseAutoFocus={(event) => { event.preventDefault(); returnFocusRef.current?.focus(); }}><DialogPrimitive.Title className="sr-only">Context inspector</DialogPrimitive.Title><ContextInspector kind={kind} onClose={onClose} /></DialogPrimitive.Content></DialogPrimitive.Portal></DialogPrimitive.Root>;
}
