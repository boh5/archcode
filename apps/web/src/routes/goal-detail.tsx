  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 px-4 h-12 border-b border-border-subtle shrink-0 bg-bg-surface">
        ...
        <span className={`text-[11px] px-2 py-0.5 rounded-sm font-medium ml-auto ${STATUS_BADGE_CLASS[goal.status]}`}>
          {goal.status}
        </span>
      </div>

      <div className="flex items-center gap-1 px-4 border-b border-border-subtle shrink-0 bg-bg-surface">
        {TABS.map((tab) => (
          <button
            key={tab.id}
