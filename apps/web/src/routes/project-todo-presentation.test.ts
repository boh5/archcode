...
describe("Project Todo presentation", () => {
  test("uses the locked card presentation precedence without changing lifecycle facts", () => {
    ...
    expect(presentProjectTodoCard({ status: "ready", hasActivation: false })).toMatchObject({ label: "Ready", tone: "neutral" });
    expect(presentProjectTodoCard({ status: "ready", hasActivation: true })).toMatchObject({ label: "In Progress", tone: "neutral" });
    expect(presentProjectTodoCard({ status: "done", hasActivation: true })).toMatchObject({ label: "Done", tone: "success" });
    expect(presentProjectTodoCard({ status: "rejected", hasActivation: true })).toMatchObject({ label: "Rejected", tone: "warning" });
  ...
  test("keeps all four workflow lane metadata in domain order", () => {
    ...
    expect(PROJECT_TODO_LANE_PRESENTATIONS.in_progress).toMatchObject({ title: "In Progress", tone: "signal" });
  });
  ...
  test("permits association motion only for real query or Session-family activity", () => {
    ...
    expect(presentProjectTodoAssociation({
      ...
      runtimeInitialized: true,
      sessionActivity: "running",
    })).toMatchObject({ tone: "signal", motion: "loop", Icon: "activity-arc" });
    expect(presentProjectTodoAssociation({
      resourceLoading: false,
      runtimeInitialized: true,
    ...
    })).toMatchObject({ Icon: Calendar, tone: "brand", motion: "none" });
    expect(presentProjectTodoAssociation({
      resourceLoading: false,
      runtimeInitialized: true,
      resourceId: "automation-1",
