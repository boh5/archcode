...
function goal(): SessionGoal {
  return {
    instanceId: "goal-1",
    generation: 1,
    objective: "Complete the migration and run tests.",
    status: "active",
    settlementReceipts: [],
    usage: {
      tokens: {
...
describe("web Session Goal query contracts", () => {
  test("fetches the global Agent catalog", async () => {
    ...
    expect(
      await (
        agentsQueryOptions() as unknown as QueryOptionWithFn<AgentDescriptor[]>
      ).queryFn(),
    ).toEqual(agents);
  });
  ...
  test("returns Session summaries with their Session-owned Goal", async () => {
    ...
    expect(
      await (
        sessionsQueryOptions(slug) as unknown as QueryOptionWithFn<
          SessionSummary[]
        >
      ).queryFn(),
    ).toEqual(sessions);
  ...
  test("keeps Diff scoped to a Session", async () => {
    ...
    expect(
      await (
        diffQueryOptions(slug, "root") as unknown as QueryOptionWithFn<
          unknown[]
        >
      ).queryFn(),
    ).toEqual([]);
