 ...
 *
 * Section order: Identity → Role → Guidelines → Tools → Environment → Memory → Project Context
 */
export async function buildSystemPrompt(ctx: PromptContext): Promise<string> {
  ...
  sections.push(buildGuidelinesSection(), buildToolSection(ctx), buildEnvSection(ctx.env));
